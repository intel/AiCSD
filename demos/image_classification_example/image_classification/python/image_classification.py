...
def classify(images_list='input_images.txt', grpc_address='localhost', grpc_port=9000,input_name='input', output_name='resnet_v1_50/predictions/Reshape_1',model_name='resnet',size=224,rgb_image=0):
 try:
  ...
  args = {"images_list":images_list,"grpc_address":grpc_address,"grpc_port":grpc_port,"input_name":input_name,"output_name":output_name,"model_name":model_name,"size":size,"rgb_image":rgb_image}
  ...
  accuracy = matched/i
  accuracy_str = str(accuracy*100) + "%"
  latency_str = str(latency) + " ms"
